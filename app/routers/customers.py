...
def register_customer(customer_write: CustomerWrite, session: Session = Depends(get_session)):
    ...
    return JSONResponse(status_code=status.HTTP_201_CREATED,
                        content={"message": f"Customer {customer_read.id} successfully created",
                                 "customer_info":customer_read.dict()})
...
def customer_info(customer_id: int, session: Session = Depends(get_session)):
    ...
    return JSONResponse(status_code=status.HTTP_200_OK,
                        content={"message": f"Customer {customer_id} successfully loaded",
                                 "customer_info":customer.dict()})
